...
static struct Button {
	...
	Color normalTextClr;
	Color focusedTextClr;
	Color clickedTextClr;
	...
	unsigned int camFlags, char layer, ButtonResponse pressResponse, ButtonResponse releaseResponse )
{
	...
	buttons[newIdx].normalTextClr = normalTextClr;
	buttons[newIdx].focusedTextClr = focusedTextClr;
	buttons[newIdx].clickedTextClr = clickedTextClr;
...
void btn_Draw( void )
{
	int i;
	int img = -1;
	Color textClr = CLR_WHITE;

	for( i = 0; i < MAX_BUTTONS; ++i ) {
		if( buttons[i].inUse ) {
			switch( buttons[i].state ) {
			case BS_NORMAL:
				textClr = buttons[i].normalTextClr;
				img = buttons[i].normalImgId;
				break;
			case BS_FOCUSED:
				textClr = buttons[i].focusedTextClr;
				img = buttons[i].focusImgId;
				break;
			case BS_CLICKED:
				textClr = buttons[i].clickedTextClr;
			...
			if( ( buttons[i].text[0] != 0 ) && ( buttons[i].fontID >= 0 ) ) {
				txt_DisplayString( buttons[i].text, buttons[i].position, textClr, HORIZ_ALIGN_CENTER, VERT_ALIGN_CENTER,
...
{
	if( !buttons[id].inUse ) {
		SDL_LogError( SDL_LOG_CATEGORY_APPLICATION, "Attempting to focus on invalid button." );
		return;
	}
	...
	for( int i = 0; i < MAX_BUTTONS; ++i ) {
		...
		buttons[i].state = ( i == id ) ? BS_FOCUSED : BS_NORMAL;
	}
}
...
{
	if( !buttons[id].inUse ) {
		SDL_LogError( SDL_LOG_CATEGORY_APPLICATION, "Attempting to do press respond on invalid button." );
		return;
	}

	if( buttons[id].pressResponse != NULL ) {
		buttons[id].pressResponse( );
	}
}
...
{
	if( !buttons[id].inUse ) {
		SDL_LogError( SDL_LOG_CATEGORY_APPLICATION, "Attempting to do release respond on invalid button." );
		return;
	}

	if( buttons[id].releaseResponse != NULL ) {
		buttons[id].releaseResponse( );
	}
}